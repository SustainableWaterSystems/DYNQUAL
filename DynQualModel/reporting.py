...
class Reporting(object):
    ...
    def __init__(self, configuration, model, modelTime):
        ...
        self.simulatePPCPs = configuration.routingOptions['simulatePPCPs'] #for simulating TSS
        
        self.simulateTSS = configuration.routingOptions['simulateTSS'] #for simulating TSS
        if self.simulateTSS:
            self.simulateSedimentTrapping = configuration.routingOptions.get('simulateSedimentTrapping', 'False') == 'True'
    ...
    def additional_post_processing(self):
        ...
        if self._model.routing.quality:
            # water temperature (K)
            self.waterTemp = pcr.ifthen(self._model.routing.landmask,self._model.routing.waterTemp)
            
            # water height (m)
            self.waterHeight = pcr.ifthen(self._model.routing.landmask,self._model.routing.water_height)
            
            # ice thickness (m)
            self.iceThickness = pcr.ifthen(self._model.routing.landmask,self._model.routing.iceThickness)
            ...
            self.TDSload = pcr.ifthen(self._model.routing.landmask,self._model.routing.TDSload) #in grams
            self.routedTDS = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedTDS) #in grams
            self.salinity = pcr.ifthen(self._model.routing.landmask,self._model.routing.salinity) #in mg l-1
            ...
            if self.loadsPerSector == "True":
                #-TDS
                self.Dom_TDSload = pcr.ifthen(self._model.routing.landmask, self._model.routing.Dom_TDSload)
                self.Man_TDSload = pcr.ifthen(self._model.routing.landmask,self._model.routing.Man_TDSload)
                self.USR_TDSload = pcr.ifthen(self._model.routing.landmask,self._model.routing.USR_TDSload)
                self.Irr_TDSload = pcr.ifthen(self._model.routing.landmask,self._model.routing.Irr_TDSload)
                
                self.routedDomTDS = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedDomTDS)
                self.routedManTDS = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedManTDS)
                self.routedUSRTDS = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedUSRTDS)
                self.routedIrrTDS = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedIrrTDS)
            
            # Aspects related to organic pollution
            self.BODload = pcr.ifthen(self._model.routing.landmask,self._model.routing.BODload) #in grams
            self.routedBOD = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedBOD) #in grams
            self.organic = pcr.ifthen(self._model.routing.landmask,self._model.routing.organic) #in mg/l
            ...
            if self.loadsPerSector == "True":
                #-BOD
                self.Dom_BODload = pcr.ifthen(self._model.routing.landmask,self._model.routing.Dom_BODload)
                self.Man_BODload = pcr.ifthen(self._model.routing.landmask,self._model.routing.Man_BODload)
                self.USR_BODload = pcr.ifthen(self._model.routing.landmask,self._model.routing.USR_BODload)
                self.intLiv_BODload = pcr.ifthen(self._model.routing.landmask,self._model.routing.intLiv_BODload)
                self.extLiv_BODload = pcr.ifthen(self._model.routing.landmask,self._model.routing.extLiv_BODload)
                
                self.routedDomBOD = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedDomBOD)
                self.routedManBOD = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedManBOD)
                self.routedUSRBOD = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedUSRBOD)
                self.routedintLivBOD = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedintLivBOD)
                self.routedextLivBOD = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedextLivBOD)
            
            # Aspects related to dissolved oxygen
            self.dissolved_oxygen = pcr.ifthen(self._model.routing.landmask,self._model.routing.dissolved_oxygen) #in mg/l
            
            # Aspects related to pathogen pollution
            self.FCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.FCload) #in million cfu
            self.routedFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedFC) #in million cfu
            self.pathogen = pcr.ifthen(self._model.routing.landmask,self._model.routing.pathogen) # in cfu/100ml
            ...
            if self.loadsPerSector  == "True":
                #-FC
                self.Dom_FCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.Dom_FCload)
                self.Man_FCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.Man_FCload)
                self.USR_FCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.USR_FCload)
                self.intLiv_FCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.intLiv_FCload)
                self.extLiv_FCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.extLiv_FCload)
                
                self.routedDomFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedDomFC)
                self.routedManFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedManFC)
                self.routedUSRFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedUSRFC)
                self.routedintLivFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedintLivFC)
                self.routedextLivFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedextLivFC)
        
            ## Aspects related to pharmaceutical pollution
            if self._model.routing.simulatePPCPs:

                #Amoxicillin
                self.AMXload = pcr.ifthen(self._model.routing.landmask,self._model.routing.AMXload) #UPDATE
                self.routedAMX = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedAMX) #UPDATE
                self.amoxicillin = pcr.ifthen(self._model.routing.landmask,self._model.routing.amoxicillin) #UPDATE
                self.AMXflux = pcr.ifthen(self._model.routing.amoxicillin != vos.MV, self.amoxicillin * self._model.routing.disChanWaterBody) #UPDATE  
                
                #Atenolol
                self.ATLload = pcr.ifthen(self._model.routing.landmask,self._model.routing.ATLload) #UPDATE
                self.routedATL = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedATL) #UPDATE
                self.atenolol = pcr.ifthen(self._model.routing.landmask,self._model.routing.atenolol) #UPDATE
                self.ATLflux = pcr.ifthen(self._model.routing.atenolol != vos.MV, self.atenolol * self._model.routing.disChanWaterBody) #UPDATE

                #Carbamazepine
                self.CBZload = pcr.ifthen(self._model.routing.landmask,self._model.routing.CBZload) #UPDATE
                self.routedCBZ = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedCBZ) #UPDATE
                self.carbamazepine = pcr.ifthen(self._model.routing.landmask,self._model.routing.carbamazepine) #UPDATE
                self.CBZflux = pcr.ifthen(self._model.routing.carbamazepine != vos.MV, self.carbamazepine * self._model.routing.disChanWaterBody) #UPDATE

                #Diclofenac
                self.DFCload = pcr.ifthen(self._model.routing.landmask,self._model.routing.DFCload) #UPDATE
                self.routedDFC = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedDFC) #UPDATE
                self.diclofenac = pcr.ifthen(self._model.routing.landmask,self._model.routing.diclofenac) #UPDATE
                self.DFCflux = pcr.ifthen(self._model.routing.propranolol != vos.MV, self.diclofenac * self._model.routing.disChanWaterBody) #UPDATE

                #Ibuprofen
                self.IBPload = pcr.ifthen(self._model.routing.landmask,self._model.routing.IBPload) #UPDATE
                self.routedIBP = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedIBP) #UPDATE
                self.ibuprofen = pcr.ifthen(self._model.routing.landmask,self._model.routing.ibuprofen) #UPDATE
                self.IBPflux = pcr.ifthen(self._model.routing.ibuprofen != vos.MV, self.ibuprofen * self._model.routing.disChanWaterBody) #UPDATE  
                            
                #Naproxen
                self.NPXload = pcr.ifthen(self._model.routing.landmask,self._model.routing.NPXload) #UPDATE
                self.routedNPX = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedNPX) #UPDATE
                self.naproxen = pcr.ifthen(self._model.routing.landmask,self._model.routing.naproxen) #UPDATE
                self.NPXflux = pcr.ifthen(self._model.routing.naproxen != vos.MV, self.naproxen * self._model.routing.disChanWaterBody) #UPDATE
            
                #Propranolol
                self.PPLload = pcr.ifthen(self._model.routing.landmask,self._model.routing.PPLload) #UPDATE
                self.routedPPL = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedPPL) #UPDATE
                self.propranolol = pcr.ifthen(self._model.routing.landmask,self._model.routing.propranolol) #UPDATE
                self.PPLflux = pcr.ifthen(self._model.routing.propranolol != vos.MV, self.propranolol * self._model.routing.disChanWaterBody) #UPDATE
            
                #Sulfamethoxazole
                self.SMXload = pcr.ifthen(self._model.routing.landmask,self._model.routing.SMXload) #UPDATE
                self.routedSMX = pcr.ifthen(self._model.routing.landmask,self._model.routing.routedSMX) #UPDATE
                self.sulfamethoxazole = pcr.ifthen(self._model.routing.landmask,self._model.routing.sulfamethoxazole) #UPDATE
                self.SMXflux = pcr.ifthen(self._model.routing.sulfamethoxazole != vos.MV, self.sulfamethoxazole * self._model.routing.disChanWaterBody) #UPDATE
    
            # TSS (Total Suspended Sediments) variables
            if self._model.routing.simulateTSS:
                
                self.sedimentLoad = pcr.ifthen(self._model.routing.landmask, self._model.routing.sedimentLoad) #sediment delivery to water (kg/day)
                self.routedsedimentLoad = pcr.ifthen(self._model.routing.landmask, self._model.routing.routedsedimentLoad) #routed sediment load in water (kg)
                self.sedimentConcentration = pcr.ifthen(self._model.routing.landmask, self._model.routing.sedimentConcentration) #sediment concentration (kg/m3)            
                self.TSS = pcr.ifthen(self._model.routing.landmask, self._model.routing.TSS) #sediment concentration in mg/L
                self.availableSedimentStock = pcr.ifthen(self._model.routing.landmask, self._model.routing.availableSedimentStock) #available sediment stock in riverbed (kg)
                self.floodplainSedimentStock = pcr.ifthen(self._model.routing.landmask, self._model.routing.floodplainSedimentStock) #floodplain sediment stock (kg)
                self.transportCapacity = pcr.ifthen(self._model.routing.landmask, self._model.routing.transportCapacity) #transport capacity (kg/m3)
                self.unitStreamPower = pcr.ifthen(self._model.routing.landmask, self._model.routing.unitStreamPower) #unit stream power (m/s)
                self.sedimentUptake = pcr.ifthen(self._model.routing.landmask, self._model.routing.sedimentUptake) #sediment uptake/entrainment rate (kg/day)
                self.sedimentDeposition = pcr.ifthen(self._model.routing.landmask, self._model.routing.sedimentDeposition) #sdiment deposition rate (kg/day) (note. negative values indicate settling)        
    
                # Sediment trapping variables (if enabled)
                if self._model.routing.simulateSedimentTrapping:
                    self.sedimentTrappingEfficiency = pcr.ifthen(
                        self._model.routing.landmask, 
                        self._model.routing.sedimentTrappingEfficiency
                    )
                    self.sedimentTrapped = pcr.ifthen(
                        self._model.routing.landmask,
                        self._model.routing.sedimentTrapped
                    )
                    
                    self.waterBodySedimentStorage = pcr.ifthen(
                        self._model.routing.landmask,
                        self._model.routing.waterBodySedimentStorage
                    )
